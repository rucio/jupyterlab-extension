...
class DIDBrowserHandlerImpl:
    ...
    def __init__(self, namespace, rucio):
        ...
        logger.info(f"DIDBrowserHandlerImpl initialized with namespace: {namespace}")
        logger.debug(f"Namespace: {namespace}, Rucio instance: {rucio}")

    def get_files(self, scope, name, force_fetch=False):
        parent_did = f'{scope}:{name}'
        logger.info(f"Fetching files for DID: {parent_did}, force_fetch={force_fetch}")
        logger.debug(f"Scope: {scope}, Name: {name}, Force fetch: {force_fetch}")

        attached_files = self.db.get_attached_files(namespace=self.namespace, did=parent_did) if not force_fetch else None
        if attached_files:
            logger.info(f"Found cached attached files for DID: {parent_did}")
            logger.debug(f"Cached attached files: {attached_files}")
            return [d.__dict__ for d in attached_files]

        logger.info(f"No cached files found for DID: {parent_did}. Fetching from Rucio.")
        file_dids = self.rucio.get_replicas(scope, name)
        logger.debug(f"Fetched file DIDs from Rucio: {file_dids}")
        ...
        logger.info(f"Fetched and cached {len(attached_files)} files for DID: {parent_did}")
        logger.debug(f"Attached files cached: {attached_files}")
        return [d.__dict__ for d in attached_files]
...
class DIDBrowserHandler(RucioAPIHandler):
    ...
    def get(self):
        ...
        logger.info(f"Handling GET request for namespace: {namespace}, DID: {did}, poll={poll}")
        logger.debug(f"Query arguments - Namespace: {namespace}, Poll: {poll}, DID: {did}")

        rucio = self.rucio.for_instance(namespace)
        (scope, name) = did.split(':')
        logger.debug(f"Split DID into scope: {scope}, name: {name}")

        handler = DIDBrowserHandlerImpl(namespace, rucio)

        try:
            dids = handler.get_files(scope, name, poll)
            logger.info(f"Successfully fetched files for DID: {did}")
            logger.debug(f"Fetched files: {dids}")
            self.finish(json.dumps(dids))
        except RucioAuthenticationException:
            logger.error(f"Authentication error while fetching files for DID: {did}")
            self.set_status(401)
            self.finish(json.dumps({'error': 'Authentication error '}))
        except Exception as e:
            logger.error(f"An error occurred while fetching files for DID: {did}. Error: {str(e)}")
            self.set_status(500)
