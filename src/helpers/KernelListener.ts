...
export default class KernelListener {
    ...
    private onRunningChanged(sender: Session.IManager, models: Iterable<Session.IModel>): void {
        ...
        const deletedModels = [...this.commIdSet].filter(x => !modelIds.includes(x));
        ...
        deletedModels.forEach(modelId => {
            this.disconnectListeners.forEach(disconnectListener => disconnectListener(modelId));
        });
    }
}